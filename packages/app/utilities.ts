...
export const DEFAULT_PROFILE_PIC =
  "https://via.placeholder.com/600x400/1DA1F2/000000?text=developer&font=roboto";
...
const buildCdnUrl = (nft: NFT, stillPreview?: boolean) => {
  return `${process.env.NEXT_PUBLIC_BACKEND_URL}/v1/media/nft/${nft.chain_name
    }/${nft.contract_address}/${nft.token_id}${stillPreview ? "?still_preview=true&cache_key=1" : "?cache_key=1"
    }`;
};
...
export const OAUTH_REDIRECT_URI = Platform.select({
  ...
  default: `io.goatsconnect${__DEV__ ? ".development" : ""}://magic-oauth-redirect`,
});
...
export const getWebBaseURL = () => {
  return `https://${process.env.NEXT_PUBLIC_WEBSITE_DOMAIN ?? "goatsconnect.com"}`;
};
