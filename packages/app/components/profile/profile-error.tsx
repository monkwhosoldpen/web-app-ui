  ...
  return (
    <View tw="items-center justify-center px-4 pt-8">
      ...
      <View tw="mt-6 w-full max-w-xs flex-row justify-between px-4">
        ...
        {Platform.OS === "web" ? (
          <Link href="mailto:help@goatsconnect.xyz">
            <ContactBtn />
          </Link>
        ) : (
          <ContactBtn
            onPress={async () => {
              const isCanOpen = await Linking.canOpenURL(
                "mailto:help@goatsconnect.xyz"
              );
              if (isCanOpen) {
                await Linking.openURL("mailto:help@goatsconnect.xyz");
              } else {
                await Clipboard.setStringAsync("help@goatsconnect.xyz");
                Alert.alert(
