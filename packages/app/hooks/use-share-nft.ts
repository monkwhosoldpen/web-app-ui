...
export const getNFTSlug = (nft: NFT) => {
  if (nft.slug) {
    ...
    return `/nft/${findTokenChainName(nft?.chain_identifier)}/${nft?.contract_address
      }/${nft?.token_id}`;
  }
};
...
export const useShareNFT = () => {
  ...
  const shareNFTOnTwitter = async (nft?: NFT) => {
    ...
    Linking.openURL(
      getTwitterIntent({
        url,
        message: `Just collected "${nft?.token_name}" on @goatsconnect_com ✦\n\n${nft?.gating_type === "paid_nft"
          ? "Collect to unlock:"
...
export const useShareNFTOnTwitter = (nft: NFT) => {
  ...
  const shareNFTOnTwitter = useCallback(async () => {
    ...
    Linking.openURL(
      getTwitterIntent({
        url,
        message: `Just collected "${nft?.token_name
