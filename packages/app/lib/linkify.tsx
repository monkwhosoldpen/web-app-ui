...
export const linkifyDescription = (text?: string, tw?: TW) => {
  ...
  let replacedText = reactStringReplace(
    ...
    (match, i) => {
      ...
      if (parsed.isIcann || match.startsWith("http:")) {
        ...
        // check if the domain is goatsconnect.com
        if (parsed.domain === "goatsconnect.com") {
          match = match.replace(
            /(www\.|http:\/\/|https:\/\/|goatsconnect\.com)/gi,
            ""
          );
        }
  ...
  replacedText = reactStringReplace(replacedText, /@(\w+)/g, (match, i) => (
    <TextLink
      href={`https://twitter.com/${match}`} // Prepending Twitter's domain here
      key={match + i}
      target="_blank"
      title="Open profile"
      // @ts-expect-error TODO: fix this
